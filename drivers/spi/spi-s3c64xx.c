...
#define S3C64XX_SPI_DEFAULT_TIMEOUT	(200)
>>>>>>> upstream/android-13
...
=======
#define S3C64XX_SPI_SLAVE_NSC_CNT_2		0 << 4 //(2<<4)
...
static void exynos_pdma_sel(struct s3c64xx_spi_driver_data *sdd);
...
static int s3c64xx_spi_dma_tx_status(struct s3c64xx_spi_driver_data *sdd,
					struct s3c64xx_spi_dma_data *dma,
					struct dma_tx_state *txstate)
{
#ifdef CONFIG_ARM64
	return sdd->ops->tx_status((unsigned long)dma->ch, dma->cookie, txstate);
#else
	return sdd->ops->tx_status((enum dma_ch)dma->ch, dma->cookie, txstate);
#endif
}
...
=======
	...
	ms = max(ms, sci->timeout); /* minimum timeout */
...
=======
	...
	if (sdd->port_conf->clk_from_cmu) {
		...
		if (sdd->target_speed != sdd->cur_speed) {
			dev_info(&sdd->pdev->dev, "Set SPI clock rate: %u(%lu)\n",
					sdd->cur_speed, clk_get_rate(sdd->src_clk));
			sdd->target_speed = sdd->cur_speed;
		}
		sdd->cur_speed = clk_get_rate(sdd->src_clk) / 4;
>>>>>>> upstream/android-13
	} else {
		/* Configure Clock */
		val = readl(regs + S3C64XX_SPI_CLK_CFG);
...
=======
		...
		if (status) {
			dev_err(&spi->dev, "I/O Error: rx-%d tx-%d res:rx-%c tx-%c len-%d dma-%d res-(%d)\\n",
				xfer->rx_buf ? 1 : 0, xfer->tx_buf ? 1 : 0,
				(sdd->state & RXBUSY) ? 'f' : 'p',
				(sdd->state & TXBUSY) ? 'f' : 'p',
			...
			if (use_dma) {
				...
				if (xfer->tx_buf != NULL
					...
					ret = s3c64xx_spi_dma_tx_status(sdd, &sdd->tx_dma, &s);
					s3c64xx_spi_dma_stop(sdd, &sdd->tx_dma);
					dev_err(&spi->dev, "TX state: %d\n", ret);
					dev_err(&spi->dev, "TX residue: %d\n", s.residue);
				}
				if (xfer->rx_buf != NULL
						&& (sdd->state & RXBUSY)) {
					s3c64xx_dma_debug(sdd, &sdd->rx_dma);
					ret = s3c64xx_spi_dma_tx_status(sdd, &sdd->rx_dma, &s);
					s3c64xx_spi_dma_stop(sdd, &sdd->rx_dma);
					dev_err(&spi->dev, "RX state: %d\n", ret);
...
static void exynos_usi_init(struct s3c64xx_spi_driver_data *sdd)
{
	...
	/* Setting nCS_TIME_COUNT control(USI_OPTION[5])
	 * If this bit set as 1, nCS is driven as high during NCS_TIME_COUNT
	 * otherwise, nCS is driven as low.
	 */
	usi_opt = readl(regs + USI_OPTION);
	usi_opt |= USI_NCS_CONTROL_ON;
	writel(usi_opt, regs + USI_OPTION);
}
...
static void exynos_pdma_sel(struct s3c64xx_spi_driver_data *sdd)
{
	...
	if ((sci->dma_mode != DMA_MODE) || (sci->secure_mode == SECURE_MODE)
			|| IS_ERR_OR_NULL(sci->dma_sysreg))
		return;

	/* Check DMA Mux selection */
	regmap_read(sci->dma_sysreg, 0x0, &val);
	dev_dbg(dev, "sysreg %u\n", val);
	if (!(val & (0x1 << sci->dma_sysreg_offset_rx))) {
		dev_err(dev, "DMAC_PERIC_%d(RX) fault! reconfigure..\n",
				sci->dma_sysreg_offset_rx);
		regmap_update_bits(sci->dma_sysreg, 0x0,
				0x1 << sci->dma_sysreg_offset_rx,
				sci->dma_sysreg_value << sci->dma_sysreg_offset_rx);
	}
	if (!(val & (0x1 << sci->dma_sysreg_offset_tx))) {
		dev_err(dev, "DMAC_PERIC_%d(TX) fault! reconfigure..\n",
				sci->dma_sysreg_offset_tx);
		regmap_update_bits(sci->dma_sysreg, 0x0,
				0x1 << sci->dma_sysreg_offset_tx,
				sci->dma_sysreg_value << sci->dma_sysreg_offset_tx);
	}
>>>>>>> upstream/android-13
}
...
=======
	...
	if (of_property_read_u32(dev->of_node, "samsung,dma-offset-tx", &sci->dma_sysreg_offset_tx))
		sci->dma_sysreg_offset_tx = -ENODEV;
	if (of_property_read_u32(dev->of_node, "samsung,dma-offset-rx", &sci->dma_sysreg_offset_rx))
		sci->dma_sysreg_offset_rx = -ENODEV;
	if (of_property_read_u32(dev->of_node, "samsung,dma-value", &sci->dma_sysreg_value))
		sci->dma_sysreg_value = -ENODEV;

	sci->dma_sysreg = syscon_regmap_lookup_by_phandle(dev->of_node,
						"samsung,dma-phandle");
	if (sci->secure_mode != SECURE_MODE) {
		if (IS_ERR(sci->usi_reg)) {
			dev_dbg(dev, "no lookup for dma-phandle.\n");
		} else if (!IS_ERR_OR_NULL(sci->dma_sysreg) &&
				(sci->dma_sysreg_offset_tx >= 0) &&
				(sci->dma_sysreg_offset_rx >= 0) &&
				(sci->dma_sysreg_value >= 0)) {
			u32 val;
			regmap_update_bits(sci->dma_sysreg, 0x0,
					0x1 << sci->dma_sysreg_offset_tx,
					sci->dma_sysreg_value << sci->dma_sysreg_offset_tx);
			regmap_update_bits(sci->dma_sysreg, 0x0,
					...
					sci->dma_sysreg_value << sci->dma_sysreg_offset_rx);
			regmap_read(sci->dma_sysreg, 0x0, &val);
			dev_info(dev, "set dma sysreg %u\n", val);
		} else {
			dev_warn(dev, "dma sysreg offset or value is not configured\n");
		}
	}

	if (of_property_read_u32(dev->of_node, "samsung,spi-timeout", &sci->timeout))
		sci->timeout = S3C64XX_SPI_DEFAULT_TIMEOUT;
...
#ifdef CONFIG_CPU_IDLE
	sdd->idle_ip_index = exynos_get_idle_ip_index(dev_name(&pdev->dev), 1);
#endif
...
int stui_spi_lock(struct spi_master *spi)
{
	...
	ret = pm_runtime_get_sync(&sdd->pdev->dev);
	if (ret < 0) {
		pr_err("%s, pm_runtime_get_sync failed:%d\n", __func__, ret);
		spi_bus_unlock(spi);
	}
#endif
...
int stui_spi_unlock(struct spi_master *spi)
{
	...
	ret = pm_runtime_put_autosuspend(&sdd->pdev->dev);
	if (ret < 0) {
		pr_err("%s, pm_runtime_put_autosuspend failed:%d\n", __func__, ret);
		return ret;
	}
#endif
